...
export default function SceneViewer({ artifact }: Props) {
  ...
  useEffect(() => {
    ...
    for (const point of artifact.points) {
      ...
      const marker = new THREE.Mesh(
        new THREE.SphereGeometry(
          point.tags?.includes("vehicle") ? 0.28 : 0.14,
          12,
          12,
        ),
    ...
    const trackPoints = artifact.vehicleTrack.map(
      (point) => new THREE.Vector3(point.x, point.y + 0.24, point.z),
    );
      ...
      <div className="viewer-shell">
        <div className="canvas-empty">
          <div>
            ...
            <p>
              The reconstruction scene will appear here with camera paths,
              sparse points, and vehicle track playback.
            </p>
          </div>
        </div>
      </div>
